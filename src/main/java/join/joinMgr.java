...
public class joinMgr {
	...
	// 기부하기
	public boolean paymentAmount(int campaign_num, String user_id, int payment, String donation_type) {
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = null;
		...
		try {
			con = pool.getConnection();
			sql = "update usertbl set amount_held = amount_held - ? where user_id = ?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, payment);
			pstmt.setString(2, user_id);
			if(pstmt.executeUpdate() == 1) {
				sql = "insert into donationstbl (campaign_num, user_id, amount, donation_date, donation_type) "
						+ "values (?, ?, ?, now(), ?)";
				pstmt = con.prepareStatement(sql);
				pstmt.setInt(1, campaign_num);
				pstmt.setString(2, user_id);
				pstmt.setInt(3, payment);
				pstmt.setString(4, donation_type);
				if(pstmt.executeUpdate() == 1) {
					flag = true;					
				}
			}
