...
<jsp:useBean id="dMgr" class="donation.DonationMgr"></jsp:useBean>
<%
	String user_id = (String)session.getAttribute("idKey");
	int subnum = Integer.parseInt(request.getParameter("subnum"));
	String donation_type = request.getParameter("donation_type");
	int amount_held = jMgr.amountHeld(user_id);
    String[] getTitleCompany = new String[2];
    getTitleCompany = dMgr.getTitleCompany(subnum, donation_type);
	if(user_id == null){
		%>
		<script>
...
<html lang="ko">
  ...
  <body>
    ...
    <div id="wrap">
      <section id="payment">
        <div class="title_wrap w_max">
          <h2 class="payment_title">
            <%= getTitleCompany[0] %>
          </h2>
          <p class="host">by <span><%= getTitleCompany[1] %></span></p>
          <button
            type="button"
            class="page_back"
        ...
        <section id="payment_form_wrap" class="w_max" style="margin-top: -30px;">
          <div class="title_wrap">
            <h2 class="slogan">"기부자님의 소중한 마음으로 놀라운 변화가 일어납니다"</h2>
            <p class="sub_slogan">투명한 기부 후기로 그 변화를 소개하고 보답하겠습니다.</p>
          </div>
          <!-- 보유금액 기부 form -->
          <form action="paymentProc.jsp?subnum=<%= subnum %>&donation_type=<%= donation_type %>" name="paymentFrm" method="post" id="amount_pay_form" autocomplete="off">
            <ul class="pay_list">
              <li class="bean_info_frame">
                <h3>보유 금액</h3>
                <strong class="current_bean">500</strong>원
                ...
                <div class="bean_desc_box" style="display: none;">
              ...
              <li class="pay_choose_frame">
                ...
                <input type="checkbox" name="choose_amount" id="fullAmount" class="sr-only" onclick="updateValue('<%= amount_held %>')">
                <label for="fullAmount">전액 기부하기</label>
              </li>
              <li class="pay_notice_frame">
                <h4 class="notice">결제수수료 없이 100% 기부</h4>
                결제하신 금액은 기부시 별도 수수료 없이 <strong>단체로 100% 기부</strong>됩니다.
              </li>
              <li class="agree_frame">
                <ul class="agree_list">
                  <li>                    
                      <input type="checkbox" name="agree" id="all_agree" class="" value="selectAll" onclick="selectAll(this)" style="margin-left:-20px; opacity:0;">
                      <label for="all_agree">이용약관, 마케팅 알림 수신에 모두 동의합니다.</label>                    
                  </li>
                  <li>
                    <input type="checkbox" name="agree" id="terms_agree" class="agree_check" value="동의" required onclick="toggleAllAgree()" style="margin-left:-20px; opacity:0;">
    ...
    &lt;부칙&gt;
      ...
      적용 일자 : 2024.01.01
                  ...
                  <li>
                    <input type="checkbox" name="agree" id="ad_agree" class="agree_check" value="동의" onclick="toggleAllAgree()" style="margin-left:-20px; opacity:0;"/>
            ...
            <div class="btn_box">
              <button type="button" onClick="check_input(paymentFrm.payBean.value)">기부하기</button>
    ...
    <script>      
      ...
      function check_input(payBean){
    	  	if(payBean == null || payBean == 0 || payBean == ""){
    	  		...
    	  		document.paymentFrm.payBean.focus();
    	  		return;
    	  	}
    		...
    		if(flag){
    			alert("숫자만 입력해 주세요.");
    		    document.paymentFrm.payBean.value = "";
    		    document.paymentFrm.payBean.focus();
    			return;
    		}
    		
    	  	if(payBean > amount_held){
    	  		alert("보유 금액보다 많이 입력했습니다.");
    	  		document.paymentFrm.payBean.focus();
    	  		return;
    	  	} 	  	
    		...
    		document.paymentFrm.payBean.value = payBean;
    		paymentFrm.submit();
      }
